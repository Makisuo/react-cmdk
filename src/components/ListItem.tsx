...
function getListItemWrapperStyles(selected: boolean, disabled?: boolean) {
	return classNames(
		...
		selected && !disabled ? 'bg-gray-200/50 dark:bg-gray-800' : 'bg-transparent',
		disabled ? 'cursor-default pointer-events-none opacity-50' : 'cursor-pointer',
	)
}
...
export function Link({
	...
	const styles = classNames(getListItemWrapperStyles(selected === index, disabled), className)
...
export function Button({
	...
	return (
		<button
			...
			data-close-on-select={closeOnSelect}
			onClick={clickAndClose}
			className={classNames(getListItemWrapperStyles(selected === index, rest.disabled), className)}
		>
...
function ListItemContent({ icon: ListItemIcon, children, type }: ListItemContentProps) {
	return (
		<>
