...
import { OpenContext, PageContext, RenderLinkContext, SearchContext, SelectContext } from '../lib/context'
...
function CommandPalette({
	...
	function handleSelect() {
		const items = document.querySelectorAll('.command-palette-list-item') as NodeListOf<
		...
		if (item) {
			...
			if (item.attributes.getNamedItem('data-close-on-select')?.value === 'true') {
				onChangeOpen(false)
			}
		}
	}
									...
									<Dialog.Panel className='w-full max-h-full bg-white dark:bg-gray-900 shadow-lg rounded-lg max-w-xl flex flex-col overflow-hidden divide-y dark:divide-gray-800'>
										...
										>
											<OpenContext.Provider value={{ isOpen, onChangeOpen }}>
												<PageContext.Provider value={{ page, searchPrefix, setSearchPrefix }}>
